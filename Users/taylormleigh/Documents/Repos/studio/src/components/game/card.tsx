...
export function Card({ card, isSelected, isHighlighted, className, onClick, draggable, onDragStart, onDragEnd, style, isStacked, "data-testid": dataTestId = 'card-undefined' }: CardProps) {
  ...
  const baseClasses = cn(
    cardSize,
    'rounded-md transition-all card-border overflow-hidden',
    ringClass,
    className
  );
    ...
    >
      ...
      >
        <div className="flex items-center gap-1">
          ...
          <div className="font-bold leading-none" style={{ fontSize: 'var(--card-rank-font-size)' }}>
            {card.rank}
          </div>

          {/* stacked face up card suite icon */}
          {isStacked && <SuitIcon suit={card.suit} style={{ fontSize: 'var(--card-stacked-suit-font-size)' }} />}
        </div>
        ...
        {!isStacked && (
          <>
            ...
            <div className="self-center flex">
              <SuitIcon suit={card.suit} style={{ fontSize: 'var(--card-suit-font-size)' }} />
            </div>
            <div>
              {/* lower right-hand rank */}
              <div className="hidden xs:hidden md:flex self-end rotate-180">
                <div className="font-bold leading-none" style={{ fontSize: 'var(--card-rank-font-size)' }}>{card.rank}</div>
              </div>
            </div>
          </>
        )}
